...
EXPECTED = {
    "usr/bin/bashref",
    "usr/lib/bashref/bashref/__main__.py",
    "usr/lib/bashref/bashref/data/manifest.json",
...
def test_launcher_uses_packaged_runtime(tmp_path: Path):
    root = stage_deb(tmp_path / "root", Path("dist/fake.whl"), "2.0.0", dry_run=True)
    launcher = (root / "usr/bin/bashref").read_text(encoding="utf-8")
    assert "PYTHONPATH=/usr/lib/bashref" in launcher
    assert 'python3 -m bashref "$@"' in launcher
